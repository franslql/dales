...
!> Reads the namelists and initialises the soil.
  subroutine initsurface
    ...
    use mpi
    use modmpi,     only : myid, myidx, myidy, comm3d, mpierr, my_real, mpi_logical, mpi_integer
    use netcdf
    ...
    call MPI_BCAST(ltskininp                  ,            1, MPI_LOGICAL, 0, comm3d, mpierr)
...
!> Calculates the interaction with the soil, the surface temperature and humidity, and finally the surface fluxes.
  subroutine surface
    use modglobal,  only : i1,j1,fkar,zf,cu,cv,nsv,ijtot,rd,rv,lopenbc,lboundary,lperiodic,rtimee,rdt
